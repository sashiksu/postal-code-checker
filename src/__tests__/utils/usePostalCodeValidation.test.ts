...
describe("usePostalCodeValidation()", () => {
  ...
  describe("Canada (CA) — Canada Post letter rules (issue #29)", () => {
    it.each([
      ...
      ["T2X 1V4"],
      ["V6B 4Y8"],
    ])("Should accept valid Canadian postal code %s", (code) => {
    ...
    it.each([
      ...
      ["W1A 0T6"],
      ["Z1A 0T6"],
    ])("Should reject invalid first letter in %s", (code) => {
    ...
    it.each([
      ...
      ["K1Q 0T6"],
      ["K1U 0T6"],
    ])("Should reject disallowed letter in 3rd position %s", (code) => {
      expect(validatePostalCode("CA", code)).toBe(false);
    });
    ...
    it.each([
      ...
      ["K1A 0Q6"],
      ["K1A 0U6"],
    ])("Should reject disallowed letter in 5th position %s", (code) => {
      expect(validatePostalCode("CA", code)).toBe(false);
    });
